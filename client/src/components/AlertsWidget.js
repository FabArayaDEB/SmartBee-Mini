                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-gray-900">
                        {alert.alerta_nombre || 'Alerta'}
                      </p>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500">
                          {moment(alert.fecha).fromNow()}
                        </span>
                      </div>
                    </div>
                    
                    <p className="mt-1 text-sm text-gray-700">
                      {alert.alerta_descripcion || 'Sin descripción'}
                    </p>
                    
                    {alert.nodo_descripcion && (
                      <p className="mt-1 text-xs text-gray-500">
                        Nodo: {alert.nodo_descripcion}
                      </p>
                    )}
                    
                    <div className="mt-2 flex items-center space-x-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        alert.indicador?.includes('CRIT') ? 'bg-danger-100 text-danger-800' :
                        alert.indicador?.includes('HIGH') ? 'bg-warning-100 text-warning-800' :
                        alert.indicador?.includes('MED') ? 'bg-yellow-100 text-yellow-800' :
                        'bg-blue-100 text-blue-800'
                      }`}>
                        {alert.indicador?.includes('CRIT') ? 'Crítica' :
                         alert.indicador?.includes('HIGH') ? 'Alta' :
                         alert.indicador?.includes('MED') ? 'Media' :
                         'Normal'}
                      </span>
